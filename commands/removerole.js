...
module.exports = {
    name: 'removerole',
    aliases: ['rrole'],
    async execute(client, message, args) {    if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      ...
      try {
        ...
        const embed = new EmbedBuilder()
        .setColor('#F6CF57')
        .setDescription(`⚠️ An error occurred while removing the role.`);
        return message.channel.send({ embeds: [embed] });
      }
    }
}
