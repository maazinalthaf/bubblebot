...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!wordStats[guildId] || !wordStats[guildId].wordCounts || Object.keys(wordStats[guildId].wordCounts).length === 0) {        // Get top words
        const topWords = Object.entries(wordStats[guildId].wordCounts)
            .sort(([,a], [,b]) => b - a)
            .slice(0, limit)
            .map(([word, count], index) => ({ rank: index + 1, word, count }));        const description = topWords.map(({ rank, word, count }) => 
            `**${rank}.** \`${word}\` - **${count}** time${count !== 1 ? 's' : ''}`
        ).join('\n');
        
        const totalWords = Object.values(wordStats[guildId].wordCounts).reduce((sum, count) => sum + count, 0);
        const uniqueWords = Object.keys(wordStats[guildId].wordCounts).length;        const embed = new EmbedBuilder()
            .setColor(embed_color)
            .setTitle(`📊 Most Common Words in ${message.guild.name}`)
            .setDescription(description)
            .addFields(
            ...
            .setTimestamp();        await message.reply({ embeds: [embed], allowedMentions: { repliedUser: false } });